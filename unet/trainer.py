...
#             validation_dataset = validation_dataset.map(utils.crop_labels_to_shape(prediction_shape)).batch(batch_size)
        ...
        train_dataset = train_dataset.map(utils.expand_dims_([w, h])).batch(batch_size)

        if validation_dataset:
            validation_dataset = validation_dataset.map(utils.expand_dims_([w, h])).batch(batch_size)
...
#                                           validation_dataset)
        ...
        self.evaluate(model, test_dataset, [w, h])
